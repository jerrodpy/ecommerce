  ...
  <div>
    ...
    <div class="card mb-4">
      <div class="card-body">
        <div class="row align-items-center">
          <div class="col-md-3">
            <label class="form-label fw-bold">Фільтр за статусом:</label>
          ...
          <div class="col-md-9 text-end">
            <div class="d-flex gap-2 justify-content-end flex-wrap">
              <span class="badge bg-secondary">Всього: {{ orders.length }}</span>
              <span class="badge bg-warning text-dark">Нових: {{ countByStatus('New') }}</span>
              <span class="badge bg-info">В обробці: {{ countByStatus('Processing') }}</span>
              <span class="badge bg-success">Виконано: {{ countByStatus('Completed') }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-else class="card mb-4">
      <div class="card-header bg-primary text-white">
        <h5 class="mb-0"><i class="bi bi-clipboard-check"></i> Список замовлень</h5>
      </div>
          ...
          <table class="table table-hover mb-0">
            ...
            <tbody>
              <tr v-if="filteredOrders.length === 0">
                <td colspan="6" class="text-center text-muted py-4">Замовлень не знайдено</td>
              </tr>
              <tr
          ...
          <div class="modal-content">
            <div class="modal-header bg-success text-white">
              <h5 class="modal-title">
                <i class="bi bi-file-earmark-text"></i> Деталі замовлення #{{ selectedOrder.id }}
              </h5>
              <button @click="selectedOrder = null" class="btn-close btn-close-white"></button>
            </div>
            <div class="modal-body">
              ...
              <h6 class="text-muted mb-3">Товари в замовленні</h6>
              ...
              <h6 class="text-muted mb-3">Коментарі</h6>
              <div class="mb-3">
                <div v-if="selectedOrder.comments" class="comment-item mb-2">
                  {{ selectedOrder.comments }}
                </div>
                <div v-else class="text-muted">Коментарів ще немає</div>
              </div>
